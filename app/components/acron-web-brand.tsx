type AcronWebTone = 'brand' | 'avelora';

type AcronWebIconProps = {
  className?: string;
  gradientId?: string;
  tone?: AcronWebTone;
};

const iconGradients: Record<AcronWebTone, [string, string, string]> = {
  brand: ['#38bdf8', '#0ea5e9', '#0284c7'],
  avelora: ['#cdb08e', '#b8966e', '#9a7a58'],
};

export function AcronWebIcon({
  className,
  gradientId = 'awg-geo',
  tone = 'brand',
}: AcronWebIconProps) {
  const [g0, g1, g2] = iconGradients[tone];
...
export function AcronWebWordmark({
  variant = 'light',
  tone = 'brand',
  compact = false,
  className = '',
}: AcronWebWordmarkProps) {
  const toneClass = tone === 'avelora' ? 'acronweb-wordmark--avelora' : `acronweb-wordmark--${variant}`;

  if (compact) {
    return (
      <span className={`acronweb-wordmark ${toneClass} ${className}`.trim()}>
        <span className="acronweb-wordmark__acron">ACRON</span>
        <span className="acronweb-wordmark__web">WEB</span>
      </span>
    );
  }
  ...
  return (
    <span className={`acronweb-wordmark ${toneClass} ${className}`.trim()}>
      {['A', 'C', 'R', 'O', 'N'].map((c) => (
        <span key={c} className="acronweb-wordmark__acron">
          {c}
        </span>
      ))}
      {['W', 'E', 'B'].map((c) => (
        <span key={c} className="acronweb-wordmark__web">
          {c}
        </span>
      ))}
    </span>
